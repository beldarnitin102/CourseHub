...
export default function Courses() {
  ...
  const [loading, setLoading] = useState(true); // 1. Initialize loading state to true
  ...
  const fetchCourses = async () => {
    ...
    try {
      ...
      setLoading(false); // 3. Turn off loading whether request succeeds or fails
    }
  };
    ...
    <div>
      <section className="min-h-screen bg-[#F3F4F6] py-20">
        <div className="mx-auto max-w-7xl px-6">
          <h1 className="mb-10 text-5xl font-bold"> Explore Courses </h1>

          {/* 4. Conditional rendering ladder: Loading -> No Courses -> Show Courses */}
          {loading ? (
            <Loader/>
          ) : courses.length === 0 ? (
